...
public class UserController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<List<Project>> userTasks(@PathVariable Long id) {
    ...
    @PostMapping("/")
    public ResponseEntity<?> addTask(@RequestBody Project task) {
        ...
        return userRepository.findByEmail(email)
                .map(user -> {
                    ...
                    task.setUser(user);
                    return new ResponseEntity<>(projectRepository.save(task), HttpStatus.CREATED);
                })
                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @PatchMapping("/")
    public ResponseEntity<?> updateTask(@RequestBody Project task) {
        Project taskToUpdate = projectRepository.findById(task.getId()).orElse(null);
